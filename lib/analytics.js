...
export async function publicSummary() {
  const [row] = await sql`
    SELECT
      ...
      (SELECT COUNT(*)::int FROM matches WHERE status = 'reported') AS matches,
      (SELECT COALESCE(SUM(home_score + away_score), 0)::int
         FROM matches WHERE status = 'reported') AS goals
  `;
  return row;
}
