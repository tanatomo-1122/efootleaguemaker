...
export async function getLeague(leagueId) {
  const [row] = await sql`
    SELECT l.*, ou.efootball_id AS organizer_efootball_id
    FROM leagues l
...
export async function listLeagues() {
  const rows = await sql`
    SELECT l.*,
           ou.efootball_id AS organizer_efootball_id,
...
export async function listEntries(leagueId) {
  return await sql`
    SELECT e.*, u.efootball_id, u.display_name, u.photo_path AS user_photo,
...
export async function listMatches(leagueId) {
  return await sql`
    SELECT m.*,
           ...
           hu.efootball_id AS home_efootball_id,
           au.efootball_id AS away_efootball_id
    FROM matches m
...
/** 主催者かどうか（efootball ID で照合） */
export async function isOrganizer(leagueId, efootballId) {
  const league = await getLeague(leagueId);
  if (!league || !league.organizer_efootball_id) return false;
  return (
    String(league.organizer_efootball_id).toLowerCase() ===
    String(efootballId || '').trim().toLowerCase()
  );
}
...
export async function buildBracket(leagueId) {
  ...
  const seedOf = (poolIndex, rank) => {
    ...
    return {
      ...
      efootball_id: settled.get(poolIndex) ? row?.efootball_id ?? null : null,
