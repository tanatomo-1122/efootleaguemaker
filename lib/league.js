...
import { applyMatchRating, revertMatchRating, recomputeAllRatings } from './rating.js';
...
export async function listEntries(leagueId) {
  return await sql`
    SELECT e.*, u.user_name, u.photo_path AS user_photo, u.last_seen_at, u.rating,
...
export async function resetMatchByOrganizer(matchId, organizerUserId, note) {
  ...
  // EFLランクから取り除く。履歴の途中が消えるので、全体を計算し直して辻褄を合わせる
  await revertMatchRating(matchId);
