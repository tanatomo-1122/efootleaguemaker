 ...
 *
 * 入力が空のときは、ログイン Cookie にフォールバックする。
 * これにより画面側は毎回ユーザーIDを送らなくてよくなる。
 *
...
export async function authenticate(input) {
  ...
  if (!raw) {
    // 動的 import。クライアントバンドルに next/headers を巻き込まないため
    const { readSessionUserId } = await import('./session.js');
    raw = readSessionUserId() ?? '';
  }
